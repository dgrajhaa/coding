...
/**
 * REST controller for managing {@link com.inn.ticket.reservation.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
public class CustomerResource {
    ...
    public ResponseEntity<Customer> createCustomer(@RequestBody Customer customer) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/customers/" + result.getUserId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getUserId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /customers/:userId} : Updates an existing customer.
     *
     * @param userId the id of the customer to save.
    ...
    public ResponseEntity<Customer> updateCustomer(
        ...
        log.debug("REST request to update Customer : {}, {}", userId, customer);
        if (customer.getUserId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(userId, customer.getUserId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, customer.getUserId().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /customers/:userId} : Partial updates given fields of an existing customer, field will ignore if it is null
     *
     * @param userId the id of the customer to save.
     * @param customer the customer to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated customer,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/customers/{userId}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Customer> partialUpdateCustomer(
        @PathVariable(value = "userId", required = false) final Long userId,
        @RequestBody Customer customer
    ) throws URISyntaxException {
        log.debug("REST request to partial update Customer partially : {}, {}", userId, customer);
        if (customer.getUserId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(userId, customer.getUserId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, customer.getUserId().toString())
        );
    }
